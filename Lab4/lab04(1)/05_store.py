...
# TODO здесь ваш кодfor i in goods :
for i in goods :
    cost, quantity = 0, 0
    for g in range(len(store[goods[i]])):
        quantity += store[goods[i]][g]['quantity']
        cost += store[goods[i]][g]['quantity'] * store[goods[i]][g]['price']
    print(f'{i} - {quantity} шт, стоимость {cost} руб')
print()

for i in goods:
    price = 0
    quantity = 0
    for x in range(len(store[goods[i]])):
        quantity += store[goods[i]][x]['quantity']
        price += store[goods[i]][x]['quantity'] * store[goods[i]][x]['price']
